...
email is sent. Otherwise, will order the food at the specified index.

Currently, emails the restaurant of the associated food for a request.

* Also sends a request to a black-box server which initiates a phone call to food's phone number of the same message.

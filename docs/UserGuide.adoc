...
Shows a list of all foods in the address book. +
Format: `list`

=== Editing a food : `edit`

Edits an existing food in the address book. +
...
Edits the phone number and email address of the 1st food to be `91234567` and `johndoe@example.com` respectively.
* `edit 2 n/Betsy Crower t/` +
Edits the name of the 2nd food to be `Betsy Crower` and clears all existing tags.
...
* `find Betsy Tim John` +
Returns any food having names `Betsy`, `Tim`, or `John`

=== Deleting a food : `delete`

Deletes the specified food from the address book. +
Format: `delete INDEX`
...
* `list` +
`delete 2` +
Deletes the 2nd food in the address book.
* `find Betsy` +
`delete 1` +
...
* `list` +
`select 2` +
Selects the 2nd food in the address book.
* `find Betsy` +
`select 1` +

...
The user executes a new `UndoableCommand`, `delete 5`, to delete the 5th food in the address book. The current state of the address book is saved before the `delete 5` command executes. The `delete 5` command will then be pushed onto the `undoStack` (the current state is saved together with the command).
...
**  Find out which of the existing API methods in  link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`] and link:{repoURL}/src/main/java/seedu/address/model/food/Person.java[`Person`] classes can be used to implement the tag removal logic. link:{repoURL}/src/main/java/seedu/address/model/AddressBook.java[`AddressBook`] allows you to update a food, and link:{repoURL}/src/main/java/seedu/address/model/food/Person.java[`Person`] allows you to update the tags.
* Solution
...
Let's add a placeholder on all our link:{repoURL}/src/main/java/seedu/address/ui/PersonCard.java[`PersonCard`] s to display a remark for each food later.
...
We have to properly encapsulate the remark in our link:{repoURL}/src/main/java/seedu/address/model/food/Person.java[`Person`] class. Instead of just using a `String`, let's follow the conventional class structure that the codebase already uses by adding a `Remark` class.
...
Now we have the `Remark` class, we need to actually use it inside link:{repoURL}/src/main/java/seedu/address/model/food/Person.java[`Person`].

**Main:**

. Add `getRemark()` in link:{repoURL}/src/main/java/seedu/address/model/food/Person.java[`Person`].
...
Since `Person` can now have a `Remark`, we should add a helper method to link:{repoURL}/src/test/java/seedu/address/testutil/PersonBuilder.java[`PersonBuilder`], so that users are able to create remarks when building a link:{repoURL}/src/main/java/seedu/address/model/food/Person.java[`Person`].

**Tests:**

. Add a new method `withRemark()` for link:{repoURL}/src/test/java/seedu/address/testutil/PersonBuilder.java[`PersonBuilder`]. This method will create a new `Remark` for the food that it is currently building.
...
|`* * *` |user |Use cmdline to add food that I like | I can get recommended meals that I will likely enjoy

|`* * *` |user |delete a food | I can avoid meals that I will probably not enjoy

|`* * *` |user |find a food by name | to see how many times I’ve ordered certain foods

|`* * *` |user |Add my allergies | I can eat safely and not die

|`* * *` |user | Be blocked from adding food preferences | Be told if the food cannot be accepted due to allergies and not die

|`* * *` |user | Edit my food preferences | I can have meals which fit my current requirements

|`* * *` |user | Contact support | Fix any issues that may happen with the application

|`* * *` |user | Can use Unix-y commands to do actions | I am not dis-immersed from coding-work when ordering food

|`* * *` |user |Pre-fill/browse local area/people nearby food address books/playlists | Save time building my own book

|`* *` |user | Rate a meal on a scale from 1 to 5 | I can get recommended meals that I will likely enjoy

|`* *` |user | Set a price range for orders | to help with budgeting

|`* *` |user | Add shortcuts for my favourite commands | Increase the speed in which I order food

|`* *` |user | See how much calories in each meal | to better track and align my personal health goals

|`* *` |user | Add culture tags to the food items that I like | The app can recommend certain types of cultural dishes

|`* *` |user | Discover new restaurants | I can try new types of cuisine

|`* *` |user | order a random meal that I will probably like to my doorstep | I can minimise distraction of food ordering and focus on my work

|`* *` |user | See images of how the food looks like | So that I know how a dish appears if I have never heard of the name before

|`* *` |user | Type in my mood and get a list of recommended foods | I can decide more quickly on what to eat

|`* *` |user |hide <<private-contact-detail,private contact details>> by default |minimize chance of someone else seeing the food by accident

|`* *` |user |Use different criterias to help decide where my food will be ordered from |I can order food based on my current needs e.g. fastest chicken rice(if I am starving)

|`* *` |user |Set a maximum number of food or restaurants to be shown |I will not be overloaded with information

|`* *` |user |Search, add and order for foods with a sale or discounted price | Save money and get good value for money

|`*` |user |Check how long my food will take to arrive |I have an idea of the waiting time

|`*` |user |Save restaurants that I like |I can order food from my preferred restaurants more easily

|`*` |user |Set personal goals |I can get recommended meals which are aligned with my objectives

|`*` |user |Add friends using the app |So I can know what they’re eating and talk about it

|`*` |user |See the delivery rate of the deliverer |I can estimate the probability of the chances of my food arriving

|`*` |user |See the rating of a restaurant |I know how reliable/how good the food is from that restaurant

|`*` |user |Delete restaurants that I have saved | So that I can keep an updated list of restaurants I like

|`*` |user with many foods in the address book |sort foods by name |locate a food easily

|`*` |user |See a history of what I’ve written as a GitHub-style diagram |I can see something cool
|=======================================================================
...
+
[none]
** 1a1. Hackeat informs user that nothing is on sale.
** 1a2. User chooses to order other food.
+

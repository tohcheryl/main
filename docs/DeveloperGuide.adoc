...
==== Motivation
Allowing a user to change his/her profile picture is the first step to making his/her experience
with HackEat a personalized one.

==== Current Implementation
A default profile picture is downloaded from the Internet when HackEat is first started and this is
stored in the same directory as the jar file.
When the command is called, the user selects an appropriate image.
The image overwrites the existing profile picture.
An event is then raised and this causes the image displayed to change.
...
** Cons: Restricts this application to a single user.
...
Since our app is targeted only at a user i.e. the person who installs and sets up HackEat on his computer,
 such an implementation was unnecessary and Alternative 2 was chosen instead.
...
==== Motivation
Allows a user to update their details easily without having to provide all the compulsory details again.
...
.. The existing user profile is retrieved.
...
** Pros: It was more direct to change the details immediately from the Logic Component.

** Cons: It can result in increased coupling among the classes.

* **Alternative 2 (Current choice):** Creation of a new EditUserDescriptor object

** Pros: A cleaner implementation where there is less coupling.

** Cons: On first sight, it seemed more tedious to implement.
// end::edituser[]
...
.. Test case: `changepic`
   Expected: A file browser will pop up. Select a _square_ image. The user profile picture +
   should be updated.
